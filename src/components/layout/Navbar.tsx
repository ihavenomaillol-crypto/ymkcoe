...
import { Menu, X, ChevronDown, Phone, Mail, Bell, Sun, Moon } from "lucide-react";
            ...
            {activeDropdown === 'dept' && (
              ...
              >
                ...
                <div className="w-full bg-background border border-border rounded-xl shadow-xl p-2 flex flex-col gap-1 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <Link href="/departments" onClick={() => setActiveDropdown(null)}>
                    <span className="block px-3 py-2 mb-1 rounded-lg text-xs font-semibold text-accent hover:bg-muted transition-colors">
                      View all programs
                    </span>
                  </Link>
                        ...
                        >
                          ...
                          {activeSubDropdown === item.label && (
                            <div 
                              onMouseEnter={() => openSubMenu(item.label)}
                              onMouseLeave={closeSubMenu}
                              className="absolute left-full top-0 ml-1 w-[240px] z-50"
                            >
                              <div className="w-full max-h-[350px] overflow-y-auto bg-background border border-border rounded-lg shadow-lg p-2 flex flex-col gap-1 animate-in fade-in slide-in-from-left-2 duration-200">
      ...
      {isOpen && (
        <div className="lg:hidden border-t border-border bg-background animate-in slide-in-from-top-2">
          <nav className="w-full px-4 py-4 flex flex-col space-y-2">
            ...
            <div>
              <button onClick={() => setShowMobileDept(!showMobileDept)} className="w-full flex items-center justify-between px-4 py-2 rounded-md text-base font-medium hover:bg-muted text-left">
                Departments <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${showMobileDept ? "rotate-180" : ""}`} />
              </button>
              {showMobileDept && (
                <div className="pl-4 mt-1 space-y-1">
                  <Link href="/departments">
                    <span className="block px-4 py-2 rounded-md text-sm font-semibold text-accent hover:bg-muted" onClick={() => setIsOpen(false)}>
                      All Departments
                    </span>
                  </Link>
                  {DEPARTMENTS.map((dept) => (
