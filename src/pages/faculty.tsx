...
import { DEPARTMENTS as OFFICIAL_DEPARTMENTS } from "@/lib/departments";

const DEPARTMENTS = [
  "All",
  ...OFFICIAL_DEPARTMENTS.map(d => d.label)
];
...
export default function Faculty() {
  ...
  return (
    <AppLayout>
      <section data-scroll-reveal className="bg-primary text-white py-16">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg text-primary-foreground/80 max-w-2xl">
            Learn from distinguished professors, industry veterans, and dedicated researchers committed to your success.
          </p>
        </div>
      </section>
      ...
      <section data-scroll-reveal className="py-16 bg-muted/20 min-h-[50vh]">
        <div className="container mx-auto px-4">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {sortedFaculty.map((faculty) => (
                <Card key={faculty.id} className={`overflow-hidden hover:shadow-md transition-shadow border-border ${faculty.isHOD ? 'ring-2 ring-accent ring-offset-2' : ''}`}>
                  ...
                  <CardContent className="p-5 border-t border-border/50 bg-card">
