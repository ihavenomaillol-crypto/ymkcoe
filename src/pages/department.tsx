...
const FACULTY_PLACEHOLDER = [
  { 
    name: "Dr. (To be updated)", 
    designation: "Head of Department", 
    qualification: "Ph.D.",
    quote: "\"Directs technology architecture, oversees engineering standards, and leads development across high-performance web applications.\"",
    linkedin: "#", github: "#", email: "#"
  },
  { 
    name: "Prof. (To be updated)", 
    designation: "Associate Professor", 
    qualification: "M.Tech",
    quote: "\"Focuses on advanced AI algorithms and scalable distributed systems for modern web infrastructure.\"",
    linkedin: "#", github: "#", email: "#"
  },
  { 
    name: "Prof. (To be updated)", 
    ...
    qualification: "M.E.",
    quote: "\"Specializes in cloud computing, microservices architecture, and modernizing legacy enterprise systems.\"",
    linkedin: "#", github: "#", email: "#"
  },
  { 
    name: "Prof. (To be updated)", 
    designation: "Assistant Professor", 
    qualification: "M.Tech",
    quote: "\"Researches human-computer interaction, front-end optimization, and state management in complex applications.\"",
    linkedin: "#", github: "#", email: "#"
  },
];
...
export default function Department() {
  ...
  const { data: allFaculty = [], isLoading: isFacultyLoading } = useGetFaculty();
  const officialDeptLabel = DEPARTMENTS.find(d => d.id === deptId)?.label || dept?.name;
  const departmentFaculty = allFaculty.filter(f => f.department === officialDeptLabel);

  // Fallback to placeholder if no real faculty yet (for robustness)
  const displayFaculty = departmentFaculty.length > 0 
    ? departmentFaculty 
    : FACULTY_PLACEHOLDER;
  ...
  return (
    <AppLayout>
      ...
      {/* Tab Navigation */}
      <section data-scroll-reveal className="bg-background border-b border-border sticky top-0 z-40 shadow-sm">
        <div className="container mx-auto px-4">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="h-auto bg-transparent border-0 p-0 flex gap-0 overflow-x-auto">
              {TABS.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="px-4 py-4 rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:text-accent bg-transparent text-sm font-medium text-muted-foreground hover:text-primary transition-colors whitespace-nowrap"
                >
                  {tab.label}
                </TabsTrigger>
              ))}
            ...
            <TabsContent value="faculty" className="py-10">
              <motion.div 
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {displayFaculty.map((member, i) => (
                      <FacultyCard
                        ...
                        qualification={member.qualification || "M.Tech"}
                        quote={member.quote || `Passionate about excellence in ${dept.short} engineering.`}
                        image={member.imageUrl || `/faculty-${(i % 3) + 1}.jpg`}
                        theme={theme}
                      />
                    ))}
                  </div>
